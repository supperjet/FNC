...
import {render, getDpr} from '../../common/dom.js'
import {noop, traverse, inArray, warn} from '../../common/methods.js'
...
export default {
  ...
  methods: {
    ...
    $_onColumnScrollEnd(index) {
      ...
      /* istanbul ignore next */
      this.$emit(EVENTS.CHANGE, index, activeItemIndex, this.getColumnValue(index))
    },
...
.md-picker-column-container .md-picker-column-masker{
  position: absolute;
  z-index: 2;
  left: 0;
  right: 0;
...
.md-picker-column-container .md-picker-column-masker.top{
  top: 0;
  background: linear-gradient(to top,hsla(0,0%,100%,.2),#fff);
  background: -webkit-linear-gradient(to top,hsla(0,0%,100%,.2),#fff);
}
.md-picker-column-container .md-picker-column-masker.bottom{
  bottom: 0;
  background: linear-gradient(to bottom,hsla(0,0%,100%,.2),#fff);
  background: -webkit-linear-gradient(to bottom,hsla(0,0%,100%,.2),#fff);
}
...
.column-list li.column-item.disabled{
  opacity: 0.2;
 }
</style>
