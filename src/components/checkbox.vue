<template>
    <div class="checkbox-list">
        <ul>
            <li class="ncf-checkbox flex flex-align-center flex-pack-justify" 
                :class="{'border-bottom-1px': needBorderBtm, 'selected': indexCollections.indexOf(index)!=-1, 'disabled': item.disabled}"
                @click="!item.disabled&&handlecheckboxEvent(index)"
                v-for="(item, index) in checkboxList" :key="index"
            >
                <div class="content">
                    <slot name="content">
                           <div class="name">{{item.name}}</div>
                           <div class="tip" v-if="item.tip">{{item.tip}}</div>
                    </slot>
                </div>
                <div class="label"
                     :class="[mode]"
                     v-if="!item.disabled"
                ></div>
            </li>
        </ul>
    </div>
</template>
<script>
    const COMPONENT_NAME = 'ncf-checkbox'
    const EVENTS = {
        CHECKBOX_ITEM_CLICK: 'checkbox-item-click'
    }
    export default {
        name: COMPONENT_NAME,
        props: {
            checkboxList: {
                type: Array,
                default: function() {
                    return []
                }
            },
            needBorderBtm: {
                type: Boolean,
                default: true
            },
            mode: {
                type: String,
                default: 'checkbox_1'
            }
        },
        data() {
            return {
                indexCollections: []
            }
        },
        methods: {
            handlecheckboxEvent(index) {
                if(this.indexCollections.indexOf(index) == -1) {
                    this.indexCollections.push(index)
                }else{
                    this.indexCollections.splice(this.indexCollections.indexOf(index), 1)
                }
                console.log(this.indexCollections)
                this.$emit(EVENTS.CHECKBOX_ITEM_CLICK, this.indexCollections)
            }
        },
    }
</script>
<style>
    @import '../common/common.css';
    .ncf-checkbox {
        position: relative;
        padding: 12px 15px 12px 0;
        background: #fff;
    }
    .ncf-checkbox.disabled {
        background: #fafafa;
        margin: 0;
    }
    .ncf-checkbox .content{
        margin: 0 0 0 15px;
    }
    .ncf-checkbox .content .name{
        color: #333;
        font-size: 16px;
    }
    .ncf-checkbox.disabled .content .name{
         color: #c3c3c3;
    }
    .ncf-checkbox .content .tip{
        color: #999;
        font-size: 12px;
    }
    .ncf-checkbox .label{
        width: 16px;
        height: 16px;
        background-size: contain;
        background-repeat: no-repeat;
        background-position: center center;
    }
    .ncf-checkbox.selected .label.checkbox_1 {
        animation: zoom 0.3s;
        background-image:url(data:image/png;base64,);
    }
    .ncf-checkbox .label.checkbox_2{
        border: 1px solid #ddd;
        border-radius: 50%;
    }
    .ncf-checkbox.selected .label.checkbox_2{
        border: none;
        animation: zoom 0.3s;
        background-image: url(data:image/png;base64,)
    }
    @keyframes zoom {
        from{
            transform: scale(0);
        }
        to{
            transform: scale(1);
        }
    }
</style>
